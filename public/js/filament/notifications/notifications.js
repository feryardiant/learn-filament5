;(() => {
    ...
    e.data('notificationComponent', ({ notification: t }) => ({
      ...
      configureTransitions() {
        let i = this.computedStyle.display,
          s = () => {
            ;(e.mutateDom(() => {
              ;(this.$el.style.setProperty('display', i), this.$el.style.setProperty('visibility', 'visible'))
            }),
      ...
      configureAnimations() {
        this.$el.classList.contains('fi-inline') ||
          (this.unsubscribeLivewireHook = Livewire.interceptMessage(({ message: i, onSuccess: s }) => {
                  ...
                  o(() => {
                    ...
                    r !== l &&
                      (window.matchMedia('(prefers-reduced-motion: reduce)').matches ||
                        this.$el.animate([{ transform: `translateY(${r - l}px)` }, { transform: 'translateY(0px)' }], {
      ...
      close(i = !1) {
        let s = () => window.dispatchEvent(new CustomEvent('notificationClosed', { detail: { id: t.id } }))
        if (i === !0) {
          ;((this.isShown = !1), s())
          return
        }
  ...
  var h = class {
      ...
      send() {
        return (
          window.dispatchEvent(new CustomEvent('notificationSent', { detail: { notification: this.toJSON() } })), this
        )
      }
    },
    a = class {
      ...
      dispatchTo(t, i, s) {
        return (this.dispatch(i, s), (this._dispatchDirection = 'to'), (this._dispatchToComponent = t), this)
      }
      emit(t, i) {
